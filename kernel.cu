...
void addWithCPUScalar(volatile float c[], volatile const float a[], volatile const float b[], volatile unsigned int size) {
...
#pragma loop(no_parallel)
    for (int i = 0; i < (int)(size / 4); i++) {
        ...
        __m128 s_w = _mm_setzero_ps();
        s_w = _mm_add_ps(s_w, c_w);
        s_w = _mm_add_ps(s_w, c_w);
        s_w = _mm_add_ps(s_w, c_w);
        s_w = _mm_add_ps(s_w, c_w);
        s_w = _mm_add_ps(s_w, c_w);
...
#pragma loop(no_parallel)
    for (int i = 0; i < (int)size / 8; i++) {
        ...
        __m256 s_w = _mm256_setzero_ps();
        s_w = _mm256_add_ps(s_w, c_w);
        s_w = _mm256_add_ps(s_w, c_w);
        s_w = _mm256_add_ps(s_w, c_w);
        s_w = _mm256_add_ps(s_w, c_w);
        s_w = _mm256_add_ps(s_w, c_w);
...
#pragma loop(ivdep)
    ...
    std::cout << "Duration was: " << duration.count() / loop_count << '\n';
